...
Adaptive parallel tempering algorithms automaticly tune the tempertures of the temperature ladder 
...
First, we create a nimble model with a multimodal posterior -- here there is no information in the data to remove uncertainty about the sign of the elements of centroids.
...
rModel <- nimbleModel(bugsCode,
                      constants=list(nObs=100, cholCov=chol(diag(2))),
                      inits=list(centroids=rep(-3, 2)))
...
legend("topleft", c("centroids", "y"), col=c("red","black"), pch=4, cex=2)

```

Now for some standard MCMC using nimble
...
mcmcC$run(niter=11000)

samples <- as.matrix(mcmcC$mvSamples)
summary(samples)
...
points(x=cModel$centroids[1], y=-cModel$centroids[1], col="red", pch=4, cex=3)
points(x=-cModel$centroids[1], y=-cModel$centroids[1], col="red", pch=4, cex=3)
legend("topleft", legend="posterior modes", col="red", pch=4, bg="white")

## library(coda)
## plot(as.mcmc(tail(samples, 10000)))
...
conf <- configureMCMC(cModel, nodes="centroids", monitors="centroids")
conf$removeSamplers()
...
aptR <- buildAPT(conf, Temps=1:4, ULT= 1000, print=TRUE)

aptC <- compileNimble(aptR)

aptC$run(niter=11000)
...
We can see that jumps between nodes (black lines) are freqent and that each node has been sampled (red points).
